...
const obtenerNota = asyncErrorHandler(async (req, res, next) => {
  ...
  if (nota.rowCount == 0) throw new ApplicationError(404, `No se encontró el recurso dni: ${pk.idAlumno}, materia: ${pk.idMateria}, anio: ${pk.trimestre} y anio: ${pk.anio} en el server!`)
...
const editarNota = asyncErrorHandler(async (req, res, next) => {
  ...
  if (notaEditada.rowCount == 0) throw new ApplicationError(404, `No se encontró el recurso dni: ${datosViejos.idAlumno}, materia: ${datosViejos.idMateria}, anio: ${datosViejos.trimestre} y anio: ${datosViejos.anio} en el server!`);
